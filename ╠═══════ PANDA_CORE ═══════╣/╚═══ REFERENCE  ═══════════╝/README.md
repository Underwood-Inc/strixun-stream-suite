# Strixun Stream Suite - Documentation Suite

> **Comprehensive documentation organized by topic and audience**

Welcome to the Strixun Stream Suite documentation! This suite provides complete documentation for developers, operators, and users of the platform.

---

## 📋 Documentation Structure

### 🚀 [Getting Started](./getting-started/)
**New to Strixun Stream Suite? Start here!**

- [Installation & Setup](./getting-started/setup.md)
- [Quick Start Guide](./getting-started/quick-start.md)
- [Environment Configuration](./getting-started/environment-setup.md)
- [First Steps](./getting-started/first-steps.md)

### [Architecture](./architecture/)
**System architecture and design patterns**

- [System Overview](./architecture/system-overview.md)
- [Technical Architecture](./architecture/technical-architecture.md)
- [Component Architecture](./architecture/component-architecture.md)
- [Data Flow & Communication](./architecture/data-flow.md)
- [Storage Architecture](./architecture/storage-architecture.md)
- [API Architecture](./architecture/api-architecture.md)
- [Security Architecture](./architecture/security-architecture.md)
- [Worker Organization](./architecture/worker-organization.md)

### 🔌 [API Documentation](./api/)
**Complete API reference and usage guides**

- [API Reference](./api/reference.md)
- [API Endpoints](./api/endpoints.md)
- [API Framework Guide](./api/framework-guide.md)
- [API Usage Examples](./api/examples.md)
- [Error Handling](./api/error-handling.md)
- [Rate Limits & Quotas](./api/rate-limits.md)

### ⚙️ [Services](./services/)
**Individual service documentation**

- [OTP Auth Service](./services/otp-auth-service/)
- [Customer API](./services/customer-api/)
- [URL Shortener](./services/url-shortener/)
- [Chat Signaling](./services/chat-signaling/)
- [Game API](./services/game-api/)
- [Mods API](./services/mods-api/) - [Local R2 Storage](./services/mods-api/local-r2-storage.md)
- [Twitch API](./services/twitch-api/)

### 💻 [Development](./development/)
**Development guides and best practices**

- [Development Setup](./development/setup.md)
- [Contributing Guide](./development/contributing.md)
- [Testing Guide](./development/testing.md)
- [TypeScript Conversion](./development/typescript-conversion.md)
- [Code Style & Standards](./development/code-style.md)
- [Modularization Guide](./development/modularization.md)
- **[Worker Port Mapping](../../serverless/WORKER_PORT_MAPPING.md)** - **CRITICAL:** Standard port assignments for local development

### 🚀 [Deployment](./deployment/)
**Deployment and operations**

- [Deployment Overview](./deployment/overview.md)
- [Cloudflare Workers](./deployment/cloudflare-workers.md)
- [Cloudflare Pages](./deployment/cloudflare-pages.md)
- [GitHub Actions](./deployment/github-actions.md)
- [Environment Configuration](./deployment/environment.md)
- [Monitoring & Troubleshooting](./deployment/monitoring.md)

### ★ [Security](./security/)
**Security documentation and best practices**

- [Security Overview](./security/overview.md)
- [Security Audit](./security/audit.md)
- [Encryption Guide](./security/encryption.md)
- [Authentication](./security/authentication.md)
- [Best Practices](./security/best-practices.md)

###  [Guides](./guides/)
**How-to guides and tutorials**

- [Encryption Setup](./guides/encryption-setup.md)
- [CORS Configuration](./guides/cors-configuration.md)
- [Auto-Configuration](./guides/auto-configuration.md)
- [Email Setup](./guides/email-setup.md)
- [Custom Domain Setup](./guides/custom-domain.md)
- [Storage Setup](./guides/storage-setup.md)

### 📖 [Reference](./reference/)
**Technical reference and specifications**

- [Database Schema](./reference/database-schema.md)
- [Design System](./reference/design-system.md)
- [API Standards](./reference/api-standards.md)
- [Storage Persistence](./reference/storage-persistence.md)
- [Layout Presets Spec](./reference/layout-presets.md)

### ★ [Product Documentation](../product-docs/)
**Product overview and business documentation**

- [Product Overview](../product-docs/COMPREHENSIVE_PRODUCT_OVERVIEW.md)
- [Architectural Overview](../product-docs/ARCHITECTURAL_OVERVIEW.md)

---

## 🧭 Quick Navigation by Role

### For Developers
1. Start with [Getting Started](./getting-started/)
2. Review [Architecture](./architecture/)
3. Read [API Documentation](./api/)
4. Check [Development Guide](./development/)

### For DevOps Engineers
1. Start with [Deployment](./deployment/)
2. Review [Environment Configuration](./deployment/environment.md)
3. Check [Monitoring & Troubleshooting](./deployment/monitoring.md)

### For Security Engineers
1. Start with [Security Overview](./security/overview.md)
2. Review [Security Audit](./security/audit.md)
3. Check [Best Practices](./security/best-practices.md)

### For Product Owners
1. Start with [Product Overview](../product-docs/COMPREHENSIVE_PRODUCT_OVERVIEW.md)
2. Review [Architectural Overview](../product-docs/ARCHITECTURAL_OVERVIEW.md)

---

## ★ Documentation Statistics

- **Total Documents:** 100+
- **Categories:** 9 main sections
- **Services Documented:** 7
- **API Endpoints:** 50+
- **Code Examples:** 100+

---

## 🔍 Search Documentation

Looking for something specific?

- **API Endpoints?** ★ [API Documentation](./api/)
- **Service Setup?** ★ [Services](./services/)
- **Deployment Help?** ★ [Deployment](./deployment/)
- **Security Info?** ★ [Security](./security/)
- **How-To Guides?** ★ [Guides](./guides/)

---

## ✍️ Contributing to Documentation

When updating documentation:

1. **Follow Structure** - Place files in appropriate categories
2. **Update Index** - Update this README if adding new docs
3. **Add Examples** - Include practical code examples
4. **Use Diagrams** - Add Mermaid diagrams for complex concepts
5. **Test Examples** - Ensure all code examples work

---

## ★ Documentation Standards

All documentation follows these standards:

- ✓ **User-Friendly Language** - Clear, accessible language
- ✓ **Technical Accuracy** - Complete implementation details
- ✓ **Visual Aids** - Mermaid diagrams for complex concepts
- ✓ **Code Examples** - Practical, copy-paste ready examples
- ✓ **Complete Coverage** - All features documented

---

*Last Updated: 2025-01-XX*  
*Documentation Suite Version: 1.0.0*
