...
PWA functionality is enabled in dev mode for testing:

1. Run dev server: `pnpm dev` or `pnpm dev:turbo`
2. Open in browser (must be HTTPS or localhost)
3. The install button will appear in the header when PWA criteria are met
4. Open Chrome DevTools > Application > Manifest to verify
5. Test "Add to Home Screen" functionality via in-app button or browser prompt
...
The app includes a dedicated PWA install button in the header:

- **Automatic Detection**: Button appears when `beforeinstallprompt` event fires
- **Smart Hiding**: Hidden if app is already installed (standalone mode)
- **Perfect for OBS Docks**: Provides install functionality when browser controls are limited
- **User Feedback**: Shows toast notifications and celebration effects on install

The button is styled with accent color and subtle pulse animation to draw attention.
...
- PWA dev mode is enabled in `vite.config.ts` for testing install prompts
