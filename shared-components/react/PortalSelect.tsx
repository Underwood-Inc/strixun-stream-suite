...
const ClearButton = styled.span<{ theme: PortalSelectTheme }>`
  background: none;
  border: none;
  color: ${({ theme }) => theme.colors.textSecondary};
  font-size: 0.75rem;
...
export function PortalSelect({
  ...
  // Update position on scroll/resize and when filtered results change
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Auto-focus search input and update position after render
  useEffect(() => {
    if (isOpen && searchable && searchInputRef.current) {
      setTimeout(() => {
        searchInputRef.current?.focus();
        updatePosition(); // Update after search input renders
      }, 0);
    }
