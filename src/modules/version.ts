...
export const GITHUB_PAGES_URL = 'https://streamkit.idling.app';
...
export async function checkForUpdates(): Promise<void> {
  ...
  if (statusEl) {
    ...
    statusEl.innerHTML = '<span style="color:var(--muted)">[EMOJI] Checking for updates...</span>';
  }
      ...
      if (comparison < 0) {
        ...
        statusEl.innerHTML = `
          ...
          <span style="font-size:0.9em;color:var(--muted)">
            ${localVersion} [EMOJI] ${remoteVersion}
          </span>
        `;
        ...
        statusEl.innerHTML = '<span style="color:var(--accent)">[CONFIG] Development Version</span>';
      } else {
        // Up to date
        statusEl.style.background = 'rgba(0,255,100,0.1)';
        statusEl.style.borderLeft = '3px solid var(--success)';
        statusEl.innerHTML = '<span style="color:var(--success)">[SUCCESS] Up to Date</span>';
      }
    }
