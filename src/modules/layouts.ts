    ...
    return `
      <div class="config-item">
        ...
        <div class="config-item__actions">
          <button onclick="window.Layouts?.applyLayout(${originalIndex})" class="btn-primary btn-sm">▶️ Apply</button>
          <button onclick="window.Layouts?.previewLayout(${originalIndex})" class="btn-secondary btn-sm">👁️</button>
...
export function previewLayout(index: number): void {
  ...
  for (const name of sourceNames.slice(0, 5)) {
    ...
    log(`  • ${name}: (${Math.round(s.positionX)}, ${Math.round(s.positionY)}) ${s.visible ? '👁️' : '🚫'}`, 'info');
  }
