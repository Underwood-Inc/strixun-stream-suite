...
foreach ($app in $frontendApps) {
    ...
    if (-not (Test-Path $dir)) {
        Write-Host "  ⚠ Skipping $($app.Name) (directory not found: $dir)" -ForegroundColor Yellow
        continue
    }
...
if ($failCount -eq 0) {
    ...
    Write-Host "⚠ Some keys failed to set. Please check the errors above." -ForegroundColor Yellow
}
