...
Write-Host "[SECURITY] Setting all encryption keys for frontend and backend..." -ForegroundColor Cyan
Write-Host ""
...
foreach ($app in $frontendApps) {
    ...
    try {
        ...
        Write-Host "    [SUCCESS] $($app.Name)" -ForegroundColor Green
        $successCount++
    } catch {
...
foreach ($worker in $workers) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "    [SUCCESS] $worker" -ForegroundColor Green
            $successCount++
        } else {
...
Write-Host "  [SUCCESS] Success: $successCount" -ForegroundColor Green
Write-Host "  [ERROR] Failed: $failCount" -ForegroundColor $(if ($failCount -gt 0) { "Red" } else { "Green" })
Write-Host ""

if ($failCount -eq 0) {
    Write-Host "[SUCCESS] All keys configured successfully!" -ForegroundColor Green
    Write-Host ""
