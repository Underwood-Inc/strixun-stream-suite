...
if (-not [string]::IsNullOrWhiteSpace($Services)) {
    ...
    foreach ($service in $serviceList) {
        if ($allServices -contains $service) {
            ...
            Write-Host "[WARN] Service '$service' not found, skipping" -ForegroundColor Yellow
        }
    }
} else {
...
foreach ($service in $selectedServices) {
    ...
    if (-not (Test-Path $servicePath)) {
        Write-Host "[WARN] Skipping $service (directory not found)" -ForegroundColor Yellow
        $failCount++
        continue
    }
...
if ($failCount -eq 0) {
    Write-Host "[SUCCESS] All services configured successfully!" -ForegroundColor Green
} else {
