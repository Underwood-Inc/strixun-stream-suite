...
describe('Session Management - Integration Tests (Miniflare)', () => {
  ...
  beforeAll(async () => {
    ...
    // Request OTP - use Miniflare dispatchFetch instead of HTTP fetch
    const requestRequest = new Request('http://example.com/auth/request-otp', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: testEmail }),
    });
    const requestResponse = await otpAuthService.dispatchFetch(requestRequest);
    expect(requestResponse.status).toBe(200);

    // Verify OTP
    const verifyRequest = new Request('http://example.com/auth/verify-otp', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  }, 30000); // Reduced from 60000 - Miniflare is much faster
  ...
  describe('POST /auth/logout - Session deletion', () => {
    it('should delete session and IP mapping on logout', async () => {
      ...
      const logoutRequest = new Request('http://example.com/auth/logout', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
        },
      });
      const logoutResponse = await otpAuthService.dispatchFetch(logoutRequest);

      expect(logoutResponse.status).toBe(200);
      
      // Verify session is deleted - /auth/me should fail
      const meRequest = new Request('http://example.com/auth/me', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${jwtToken}`,
        },
      });
      const meResponse = await otpAuthService.dispatchFetch(meRequest);
  ...
  describe('Customer-API Integration', () => {
    it('should always create/upsert customer with OTP email in customer-api', async () => {
      ...
      // Use customer-api Miniflare worker directly via dispatchFetch
      // The customer-api endpoint is /customer/by-email/{email}
      const customerRequest = new Request(`http://example.com/customer/by-email/${encodeURIComponent(testEmail)}`, {
        method: 'GET',
        headers: {
      ...
      const customerResponse = await customerAPI.dispatchFetch(customerRequest);
    ...
    it('should use customerId for all lookups (not email)', async () => {
      ...
      const customerRequest = new Request(`http://example.com/customer/${customerId}`, {
        method: 'GET',
        headers: {
          // Customer API requires JWT or service auth
        },
