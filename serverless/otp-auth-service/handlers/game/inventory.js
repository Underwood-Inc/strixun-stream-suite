...
async function handleGetInventory(request, env, userId, customerId) {
    try {
        ...
        const inventory = await env.OTP_AUTH_KV.get(inventoryKey, { type: 'json' }) || {
            characterId,
            maxSlots: 20,
            slots: [],
            usedSlots: 0,
        ...
        const equipment = await env.OTP_AUTH_KV.get(equipmentKey, { type: 'json' }) || {
            characterId,
            slots: {}
        };
...
async function handleAddItem(request, env, userId, customerId) {
    try {
        ...
        await env.OTP_AUTH_KV.put(itemKey, JSON.stringify({
            id: itemId,
            ...item,
        ...
        // Update inventory
        await env.OTP_AUTH_KV.put(inventoryKey, JSON.stringify(inventory), {
            expirationTtl: 60 * 60 * 24 * 365
        });
...
async function handleRemoveItem(request, env, userId, customerId) {
    try {
        ...
        // Delete item
        const itemKey = getCustomerKey(customerId, `item_${itemId}`);
        await env.OTP_AUTH_KV.delete(itemKey);
...
async function handleEquipItem(request, env, userId, customerId) {
    try {
        ...
        // Update equipment
        await env.OTP_AUTH_KV.put(equipmentKey, JSON.stringify(equipment), {
            expirationTtl: 60 * 60 * 24 * 365
        });
