...
interface CraftingSession {
    ...
    resultModifiers?: { prefixes: { name: string; tier: number }[]; suffixes: { name: string; tier: number }[] };
}
...
interface CraftedItem {
    ...
    quantity: number;
    quality: number;
    modifiers: { prefixes: { name: string; tier: number }[]; suffixes: { name: string; tier: number }[] };
}
 ...
 */
async function handleStartCrafting(request: Request, env: Env, userId: string, customerId: string | null): Promise<Response> {
    try {
        const body = await request.json() as { characterId?: string; recipeId?: string; quantity?: number; specialMaterials?: SpecialMaterial[] };
        ...
        // Store session using entity pattern
        await putEntity(env.OTP_AUTH_KV, 'otp-auth', 'crafting-session', sessionId, session, {
            expirationTtl: 60 * 60 * 24 * 7 // 7 days
        });

        // Add to character's active sessions using index
        await indexAdd(env.OTP_AUTH_KV, 'otp-auth', 'character-crafting-sessions', characterId, sessionId);
 ...
 * Authentication handled by route wrapper
 */
async function handleCollectCrafting(request: Request, env: Env, userId: string, customerId: string | null): Promise<Response> {
    try {
        ...
        // Get session using entity pattern
        const session = await getEntity<CraftingSession>(env.OTP_AUTH_KV, 'otp-auth', 'crafting-session', sessionId);
 ...
 * Authentication handled by route wrapper
 */
async function handleGetCraftingSessions(request: Request, env: Env, userId: string, customerId: string | null): Promise<Response> {
    try {
        ...
        // Get session IDs using index
        const sessionIds = await indexGet<string[]>(env.OTP_AUTH_KV, 'otp-auth', 'character-crafting-sessions', characterId) || [];

        // Load all sessions
        const sessions: CraftingSession[] = [];
        for (const sessionId of sessionIds) {
            ...
            if (session) {
                ...
                const total = completes.getTime() - started.getTime();
 ...
 */
function calculateCraftingQuality(session: CraftingSession): number {
 ...
 */
function generateCraftingModifiers(session: CraftingSession, quality: number): { prefixes: { name: string; tier: number }[]; suffixes: { name: string; tier: number }[] } {
    const modifiers: { prefixes: { name: string; tier: number }[]; suffixes: { name: string; tier: number }[] } = {
        prefixes: [],
        suffixes: []
    };
 ...
 */
function generateCraftingItems(session: CraftingSession, quality: number, modifiers: { prefixes: { name: string; tier: number }[]; suffixes: { name: string; tier: number }[] }): CraftedItem[] {
    return [{
 ...
 */
export async function handleGameCrafting(request: Request, env: Env, userId: string, customerId: string | null, action: string): Promise<Response> {
    if (action === 'start') {
