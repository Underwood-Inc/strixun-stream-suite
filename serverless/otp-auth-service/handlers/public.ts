...
import { hashEmail, getJWTSecret, hashApiKey, decryptData } from '../utils/crypto.js';
import { generateCustomerId, storeCustomer, getCustomer, getCustomerByEmail } from '../services/customer.js';
 ...
 */
export async function handlePublicSignup(request: Request, env: Env): Promise<Response> {
    try {
        ...
        const existingSignup = await env.OTP_AUTH_KV.get<SignupData>(signupKey, { type: 'json' });
        ...
        if (existingOtp) {
            ...
            const otpData = await env.OTP_AUTH_KV.get<OTPData>(existingOtp, { type: 'json' });
            if (otpData && new Date(otpData.expiresAt) > new Date()) {
                ...
                const existingSignupData = await env.OTP_AUTH_KV.get<SignupData>(signupKey, { type: 'json' });
        ...
        const otpResponseData = await otpResponse.json() as { expiresIn?: number; remaining?: number };
        return new Response(JSON.stringify({
            success: true,
 ...
 */
export async function handleVerifySignup(request: Request, env: Env): Promise<Response> {
    try {
        ...
        const signupKey = `signup_${emailHash}`;
        const signupData = await env.OTP_AUTH_KV.get<SignupData>(signupKey, { type: 'json' });
        ...
        const otpResponseData = await otpResponse.json() as { customerId?: string; userId?: string; access_token?: string; token?: string; expires_in?: number; sub?: string };
        ...
        if (!customer) {
            ...
            try {
                ...
                if (token) {
                    ...
                    if (parts.length === 3) {
                        const payload = JSON.parse(atob(parts[1])) as { customerId?: string };
        ...
        if (finalCustomerId) {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                const keys = (await env.OTP_AUTH_KV.get<ApiKeyData[]>(customerApiKeysKey, { type: 'json' })) || [];
 ...
 */
export async function handleRegisterCustomer(request: Request, env: Env): Promise<Response> {
    try {
        const body = await request.json() as { name?: string; email?: string; companyName?: string; plan?: string };
        ...
        return new Response(JSON.stringify({
            ...
            message: err.message,
            details: env.ENVIRONMENT === 'development' ? (error as Error).stack : undefined
        }), {
            status: 500,
            headers: { ...getCorsHeadersRecord(env, request), 'Content-Type': 'application/json' },
        });
...
export async function handleHealth(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        const errorResponse = new Response(JSON.stringify({ 
            ...
            details: env.ENVIRONMENT === 'development' ? err.message : undefined
        }), {
            status: 503,
            headers: { 'Content-Type': 'application/json' },
        });
 ...
 */
export async function handleHealthReady(request: Request, env: Env): Promise<Response> {
    // CRITICAL SECURITY: JWT encryption is MANDATORY for all endpoints
    // Get JWT token from request
    const authHeader = request.headers.get('Authorization');
    // CRITICAL: Trim token to ensure it matches the token used for encryption
 ...
 */
export async function handleHealthLive(request: Request, env: Env): Promise<Response> {
    // CRITICAL SECURITY: JWT encryption is MANDATORY for all endpoints
    // Get JWT token from request
    const authHeader = request.headers.get('Authorization');
    // CRITICAL: Trim token to ensure it matches the token used for encryption
