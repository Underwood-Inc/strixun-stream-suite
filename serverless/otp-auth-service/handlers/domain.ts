 ...
 */
export async function handleRequestDomainVerification(request: Request, env: Env, customerId: string): Promise<Response> {
    try {
 ...
 */
export async function handleGetDomainStatus(request: Request, env: Env, domain: string): Promise<Response> {
    try {
        const domainKey = `domain_${domain}`;
        const verificationData = await env.OTP_AUTH_KV.get<VerificationData>(domainKey, { type: 'json' });
 ...
 */
export async function handleVerifyDomain(request: Request, env: Env, customerId: string, domain: string): Promise<Response> {
    try {
        const domainKey = `domain_${domain}`;
        const verificationData = await env.OTP_AUTH_KV.get<VerificationData>(domainKey, { type: 'json' });
