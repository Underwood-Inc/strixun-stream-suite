<script lang="ts">
  ...
  function addOrigin() {
    ...
    if (invalid.length > 0) {
      error = `Invalid: ${invalid.join(', ')}. Use http:// or https:// URL only. For the downloaded test page, serve it from a local server (e.g. npx serve . or npx http-server -p 8080) and add that origin.`;
      return;
    }
    if (added.length === 0) {
      error = 'Already added';
  ...
  <div class="modal" role="dialog" tabindex="-1" aria-modal="true" aria-labelledby="origins-modal-title"
    ...
    <div class="modal__content">
      ...
      <div class="modal__body">
        ...
        <p class="modal__text modal__text--info">
          One origin per line or comma-separated. For the downloadable test page, serve it from a local web server (e.g. <code>npx serve .</code> or <code>npx http-server -p 8080</code> in the file’s folder; or use the Live Server extension in VS Code/Cursor) and add that origin (e.g. <code>http://localhost:3000</code> or <code>http://localhost:8080</code>).
        </p>
