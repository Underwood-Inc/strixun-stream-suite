 ...
 */
async function deriveKeyFromToken(token: string, salt: ArrayBuffer): Promise<CryptoKey> {
  const encoder = new TextEncoder();
 ...
 */
export async function decryptWithJWT(encryptedData: EncryptedData, token: string): Promise<unknown> {
